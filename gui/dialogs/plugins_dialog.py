...
class PluginsDialog(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        self.set_plugin_button = QtWidgets.QPushButton(
            QIcon("resources/icons/application-plus.png"), "Set as central")
        self.uninstall_button = QtWidgets.QPushButton(
            QIcon("resources/icons/minus-circle.png"), "Uninstall", self)
        self.enable_button = QtWidgets.QPushButton(
            QIcon("resources/icons/tick.png"), "Enable", self)
        self.disable_button = QtWidgets.QPushButton(
            QIcon("resources/icons/exclamation-red.png"), "Disable", self)
    ...
    def _populate_table(self):
        ...
        self.plugins_table.setRowCount(
            len(self.parent().plugin_manager.plugins))
        self.plugins_table.setHorizontalHeaderLabels(
            ["Name", "Symbolic name", "Description", "Version", "Application version", "Enabled"])
        for i, plugin in enumerate(self.parent().plugin_manager.plugins):
            ...
            enabled = QtWidgets.QTableWidgetItem(QIcon(
                "resources/icons/tick.png") if plugin.enabled else QIcon("resources/icons/cross.png"), "")
    ...
    def _on_enable_plugin(self):
        ...
        filtered_rows = list(map(lambda x: x.row(), filter(
            lambda x: self.plugins_table.column(x) == 1, selected)))
        filtered = list(map(lambda x: x.text(), filter(
            lambda x: self.plugins_table.column(x) == 1, selected)))
        ...
        for r in filtered_rows:
            self.plugins_table.setItem(r, 5, QtWidgets.QTableWidgetItem(
                QIcon("resources/icons/tick.png"), ""))
    ...
    def _on_disable_plugin(self):
        ...
        for r in filtered_rows:
            self.plugins_table.setItem(r, 5, QtWidgets.QTableWidgetItem(
                QIcon("resources/icons/cross.png"), ""))
